    ...
    <div className='absolute top-0 left-0 w-full z-10 bg-white'>
      <div className='contanier mx-auto w-full flex justify-between items-center py-4 px-8 bg-transparent'>
        ...
        <ul className=' hidden md:flex gap-7 text-black font-semibold'>
          <a href="#Header" className='cursor-pointer hover:text-indigo-700'>Home</a>
          <a href="#About" className='cursor-pointer hover:text-indigo-700'>About</a>
          <a href="#Projects" className='cursor-pointer hover:text-indigo-700'>Info</a>
          <a href="#Testimonials" className='cursor-pointer hover:text-indigo-700'>Testimonials</a>
        </ul>
        <button className='hidden md:block bg-white px-8 py-2 rounded-full'>Sign up</button>
        <img onClick={()=>setshowMobileMenu(true)} src={assets.menuicon} className='md:hidden w-7 cursor-pointer text-black' alt="" />
      </div>
      {/* --------- mobile-menu ---------- */}
      <div className={`md:hidden ${showMobileMenu ? 'fixed w-full': 'h-0 w-0'}  right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        ...
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
          ...
          <a onClick={()=>setshowMobileMenu(false)} href="#Projects" className='px-4 py-2 rounded-full inline-block'>Info</a>
